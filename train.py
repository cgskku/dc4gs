...
from gaussian_renderer import render, render_selected, network_gui
import sys
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    vp = viewpoint_cam if pipe.viz_select else None
                    render_fn = render_selected if pipe.viz_select else None
                    
                    selected_image, cnt = gaussians.densify_and_prune(opt.densify_grad_threshold, opt.densify_grad_abs_threshold, 0.005, scene.cameras_extent, size_threshold, vp, render_fn, scene.model_path, iteration)
                    if pipe.viz_select:
                        save_path = f"{dataset.model_path}/viz/select"
                        sel_path = f"{save_path}/seletced"
                        os.makedirs(sel_path, exist_ok=True)
                        image_pil = transforms.ToPILImage()(selected_image)
                        image_pil.save(f"{sel_path}/{iteration}-{cnt}.png")
                        
                        gt_path = f"{save_path}/gt"
                        os.makedirs(gt_path, exist_ok=True)
                        image_pil = transforms.ToPILImage()(gt_image)
                        image_pil.save(f"{gt_path}/{iteration}.png")
                        
                        render_path = f"{save_path}/render"
                        os.makedirs(render_path, exist_ok=True)
                        image_pil = transforms.ToPILImage()(torch.clamp(image, 0, 1))
                        image_pil.save(f"{render_path}/{iteration}.png")
