...
def train_scene(gpu, scene, factor):
    ...
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python -u train-dcc.py -s /dataset/tandt_db/{scene} -m {output_dir}/{scene} --eval --port {6009+int(gpu)} --without_bound > {output_dir}/{scene}.log"
    print(cmd)
    if not dry_run:
        os.system(cmd)
