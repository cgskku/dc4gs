...
def train_scene(gpu, scene, factor):
    ...
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python -u train.py -s /dataset/360_v2/{scene} -m {output_dir}/{scene} -i images_{factor} --eval --port {6009+int(gpu)} > {output_dir}/{scene}.log"
    print(cmd)
    if not dry_run:
        os.system(cmd)
