...
class GaussianModel:
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, vp, render_fn, model_path, iteration, N=2):
        ...
        selected_image = None
        if vp!=None:
            override_color = torch.zeros((self.get_xyz.shape[0], 3), device=self.get_xyz.device)
            override_color[selected_pts_mask] = torch.tensor([1.0,1.0,1.0], device=self.get_xyz.device)
            render_pkg = render_fn(vp, self, override_color=override_color.float())
            selected_image = render_pkg["render"]
            
            if iteration % 1000 == 0:
                selected_features_dc = self._features_dc[selected_pts_mask]
                selected_features_rest = self._features_rest[selected_pts_mask]
                
                select_color = RGB2SH(torch.tensor([1.0, 0, 0]).float().cuda())
                
                self._features_dc[selected_pts_mask] = select_color
                ...
                self._features_dc[selected_pts_mask] = selected_features_dc
                self._features_rest[selected_pts_mask] = selected_features_rest
        ...
        return selected_image, selected_pts_mask.sum()
    ...
    def densify_and_prune(self, max_grad, max_grad_abs, min_opacity, extent, max_screen_size, vp, render_fn, model_path, iteration):
        ...
        selected_img, cnt = self.densify_and_split(grads_abs, max_grad_abs, extent, vp, render_fn, model_path, iteration)
